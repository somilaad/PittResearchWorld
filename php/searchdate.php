...
function search_date() {
  ...
  $start = $_GET['from'];
  $end = $_GET['to'];
  $sql = "select name,contact_info,description,from_date,to_date,length,payment,payment_amount,keywords,email from researches where ((from_date between '$start' and '$end') or (to_date between '$start' and '$end')) and name not in(select research_name from participant_research where p_email ='$currentUser')";
  $res = mysql_query($sql);
  $i=0;
